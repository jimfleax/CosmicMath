...
CosmicMath = {
    ...
    'fluentMode': function (id, address) {
        ...
        r = document.querySelector('#response').addEventListener('keyup', function () {
            res = [];
            (document.querySelectorAll('#response').length < 1) ? document.querySelectorAll('#response').forEach((a)=>{res.push(a.value)}) : res = (document.querySelector('#response').value);
            document.querySelector("#btn").click();
            ($0 === 'correct') ? (document.querySelector("#sc").innerHTML = currentscore++, $.createSum(id,address), $.fluentMode(id, address), document.querySelector('#response').focus()) : null;
            
        });
    },
    'submit': function (id, address, res, answer, isArray = false, actionRequired = true) {
        CosmicMath.auth.check();
        if ((isArray) ? (res === 'jim.ans') : (res.includes('jim.ans'))) {
            console.log('SHOW ANSWER');
            (document.querySelectorAll("#response").length > 1) ? document.querySelectorAll("#response").forEach((elm,k)=>{elm.value = (!(typeof answer === 'object') ? answer.split(',')[k] : answer[k])}) : (document.querySelector("#response").value = answer);
            return;                    
        };
        ...
        if (isArray === true) {
            ...
            if (res === answer) {
                ...
                document.querySelectorAll("#response").forEach((l) => { l.disabled = true });
            } else
            ...
            if (parseFloat(res).toString() === parseFloat(answer).toString()) {
                (actionRequired) ? (document.querySelector('#a100').innerHTML = `<div id="d100" class="correct"><span id="s100">Correct</span><button id="nxtBtn" onclick="CosmicMath.createSum('${id}','${address}')">Next</button></div>`, document.querySelectorAll("#response").forEach((l) => { l.disabled = true })) : (output = 'correct');
            } else
                (actionRequired) ? document.querySelector('#a100').innerHTML = `<div id="d100" class="wrong"><span id="s100">Wrong</span><button id="nxtBtn" onclick="CosmicMath.createSum('${id}','${address}')">Next</button></div>` : (output = 'wrong');
    ...
    'createSum': function (input, address) {
        ...
        !(document.querySelector('#braincrunchmode')) ? (fluentModeEl = document.createElement('div'), fluentModeEl.innerHTML = `<span title="Fluent Mode" onclick="$.score(0);CosmicMath.fluentMode('${input}', '${address}');$.msg('Fluent Mode is active');" id="braincrunchmode" style="visibility: visible;"><span style=" filter: contrast(0.01); ">⚡</span></span>`, document.querySelector('body').append(fluentModeEl)) : null;
